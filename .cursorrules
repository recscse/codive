...
- **ALWAYS PREFER** the `ctxd` MCP tools for zero-token code discovery:
  1. Use `ctxd:get_repo_map` to understand the codebase structure and symbols.
  2. Use `ctxd:get_file_skeleton` to inspect file structure without dumping thousands of tokens.
  3. Use `ctxd:find_symbol` when locating function, class, or type definitions.
  4. Use `ctxd:find_callers` or `ctxd:find_references` when discovering usages or refactoring.
  5. Use `ctxd:find_tests_for` to locate corresponding unit test suites before and after making changes.
  6. Use `ctxd:pack_feature_context` to bundle complete feature entrypoints in 1 single turn.
  7. Use `ctxd:get_git_changes` to review uncommitted AST changes without noisy unified diffs.
...
3. Record significant architectural decisions using `ctxd:save_decision`.
