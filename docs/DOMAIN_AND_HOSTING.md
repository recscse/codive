...
This guide explains how to register a custom domain, host the `ctxd` landing page and docs site for **$0/month**, and set up the one-liner install scripts (`curl ... | bash` and `irm ... | iex`).
...
| :--- | :---: | :--- |
| **`ctxd.dev`** *(Recommended)* | ~$12/yr | The gold standard for developer tools (owned by Google registry, requires HTTPS by default). |
| **`ctxd.tools`** | ~$10/yr | Extremely descriptive and high availability. |
| **`ctxd.sh`** | ~$25/yr | Excellent for terminal / CLI utility branding. |
| **`ctxd.io`** | ~$35/yr | Classic tech startup TLD. |
| **`ctxd.ai`** | ~$65/yr | Premium AI positioning. |
...
1. Push your repository to GitHub: `github.com/your-username/ctxd`.
...
5. Click **Deploy**.
6. Under **Custom Domains**, add `ctxd.dev`. Cloudflare automatically provisions a free wildcard SSL certificate.

---

### Option B: Vercel (1-Click Deployment)
1. Log into [Vercel](https://vercel.com).
2. Click **Add New Project** ➔ Import your `ctxd` GitHub repository.
...
3. Under **Custom domain**, enter `ctxd.dev` and click **Save**.
...
| :--- | :--- | :--- | :---: |
| **CNAME** | `@` / `ctxd.dev` | `<your-pages-subdomain>.pages.dev` | Proxied (Orange Cloud) |
| **CNAME** | `www` | `ctxd.dev` | Proxied (Orange Cloud) |
