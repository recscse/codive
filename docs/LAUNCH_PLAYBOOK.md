# 🚀 devctx Viral Launch Playbook & Community Distribution

This playbook contains ready-to-publish copy, video storyboards, benchmarks, and community submission templates for launching `devctx`.
...
**Title**: 
`Show HN: devctx – Local SQLite + AST context engine for AI agents (cuts token costs by 80%)`

**URL**: `https://github.com/recscse/devctx`

**Text Post**:
```markdown
Hey HN,

We built devctx because we noticed a massive inefficiency in how AI coding assistants (Claude Code, Cursor, Windsurf, Antigravity) explore codebases:
...
`devctx` is a single compiled Go binary that runs a local-first Model Context Protocol (MCP) server backed by SQLite (WAL mode):
...
5. **Real-time Savings Counter (`devctx stats`)**: Shows exactly how many tokens and dollars your repo saved.

It requires zero cloud accounts, zero telemetry, and zero Docker containers.

Quick install:
- Mac/Linux: `curl -fsSL https://recscse.github.io/devctx/install.sh | bash && devctx setup`
- Windows: `irm https://recscse.github.io/devctx/install.ps1 | iex; devctx setup`

Repo: https://github.com/recscse/devctx
...
>
> We built **devctx** — an open-source MCP engine that indexes AST symbols in SQLite and cuts latency to 1 turn.
>
> ⚡ 95% fewer tokens  
> ⏱️ 5x faster turns  
...
- **Right Screen (devctx-Powered Agent)**:
  - Agent runs `devctx:pack_feature_context("auth")`
  - 2.1 seconds, 240 tokens, targeted edit applied on Turn 1!
- **Bottom Banner**: `devctx setup: Universal 1-Click MCP Auto-Configurator`

### Tweet 2 (Features):
> What makes devctx different:
>
...
> 4️⃣ `devctx map --web`: Interactive 3D/2D visual architecture graph
...
> ```
> GitHub: https://github.com/recscse/devctx
...
**Title**: `How we cut our Claude 3.5 Sonnet token bills by 70% using a local AST MCP indexer (devctx)`
...
> 
> We built `devctx` (Go + SQLite + MCP) to act as a structured AST layer:
...
> 
> It's open source and installs with `devctx setup`. Detailed benchmarks and architecture: [GitHub link]
...
> 
> `devctx` provides token-budgeted repo maps (`token_budget: 2500`) and function skeletons so local models can navigate large codebases without context degradation or hallucinations.
...
- **Repository**: `https://github.com/recscse/devctx`
