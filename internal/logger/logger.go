...
// InitLogger sets up structured logging to .devctx/devctx.log and optionally to stderr.
func InitLogger(targetDir string, verbose bool) error {
	...
	if err := os.MkdirAll(ctxdDir, 0755); err != nil {
		return fmt.Errorf("failed to create .devctx directory: %w", err)
	}

	logPath := filepath.Join(ctxdDir, "devctx.log")
