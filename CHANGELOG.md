...
### Added
- **`read_symbol` MCP tool** returns exactly one definition (doc comment, signature and full body) instead of the whole file. It accepts `Name`, `Type.Method` or `path:Name`, and suggests close names when nothing matches exactly. On the Go standard library it returned the right definition with 11–86× fewer tokens than reading the file, typically in a few milliseconds.
  - **Go:** where a definition ends comes from Go's own parser, so it's exact.
  - **Python:** the end is found by indentation.
  - **Brace languages (JS/TS, Java, C#, Rust, C/C++, …):** a brace-matching scanner finds the end. It handles strings, raw strings, comments, character literals and Rust lifetimes, braces inside headers (React destructured props, generic constraints), bodyless declarations and Allman-style braces. It agrees with Go's parser on 99.99% of ~105k standard-library declarations, and a test keeps it checked against the parser.
- **Indexes rebuild automatically when symbol extraction changes.** Each index records which version of the symbol extractor built it. When codive's extractor changes, `serve` rebuilds the index once in the background (queries keep working meanwhile), and `update` and `watch` rebuild before continuing.
...
### Fixed
- Go method signatures were stored without their receiver (`func  Start(...)` instead of `func (s *Server) Start(...)`), so agents couldn't tell same-named methods on different types apart. Existing indexes are rebuilt automatically to pick up the fix.
